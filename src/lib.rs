...
use crate::engine::resources::{provider::*, GlobalResources};
use crate::engine::window_state::WindowState;

pub struct GraphicalApplication<
    T: Texture2DProvider
        + TextureFromImageProvider
        + BillboardProvider
        + ShaderProvider
        + GLTFModelProvider,
> {
...
impl<
        T: Texture2DProvider
            ...
            + ShaderProvider
            + GLTFModelProvider,
    > ApplicationHandler for GraphicalApplication<T>
{
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if !self.initialized {
            self.resource_provider.load_shader_into_resources(
            ...
            self.resource_provider.load_shader_into_resources(
                "shaders/billboard.wgsl",
                "billboard".to_string(),
                &self.device,
                self.resources.clone(),
            );
            self.resource_provider.load_shader_into_resources(
                "shaders/default_srgb.wgsl",
                "default_srgb".to_string(),
                &self.device,
                self.resources.clone(),
            );
            self.resource_provider.load_shader_into_resources(
                "shaders/billboard_srgb.wgsl",
                "billboard_srgb".to_string(),
                &self.device,
                self.resources.clone(),
            );

            let texture = self.resource_provider.load_texture_from_image(
                "example/test.png",
                &self.device,
                &self.queue,
            );
            let billboard = self.resource_provider.create_billboard(
                [0.4, 0.4],
                Some(texture.clone()),
                &self.device,
            );
            ...
            let mut model_meshes = self
                .resource_provider
                .load_model("res/cube_test.glb", &self.device)
                .unwrap();
    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                new_camera_view.eye = cgmath::Point3::new(
                    (self.time_passed / 5.0).sin() as f32 * 0.6,
                    0.15,
                    (self.time_passed / 5.0).cos() as f32 * 0.6,
                );
                state.set_camera_view(new_camera_view);

                let resources = self.resources.try_write();
                if let Ok(mut resources) = resources {
                    resources.meshes[1].set_transform(Transform::new(
                        [0.0, (self.time_passed).sin() as f32 * 0.2, 0.0],
                        cgmath::Quaternion::new(0.0, 0.0, 0.0, 1.0).into(),
                        [1.0, 1.0, 1.0],
                    ));
                }
